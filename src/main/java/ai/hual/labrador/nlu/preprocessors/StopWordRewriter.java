...
@Component("stopWordRewriter")
public class StopWordRewriter implements Preprocessor {

    private static final String STOP_WORDS_FILE = "stop_words.txt";
    private Trie stopWordTrie;

    private static Logger logger = LoggerFactory.getLogger(StopWordRewriter.class);

    public StopWordRewriter() {
        logger.debug("Constructing stop words trie from file: {}", STOP_WORDS_FILE);
        stopWordTrie = new VanillaTrie();
        List<String> stopWords = new StringUtils().fetchWords(STOP_WORDS_FILE);
        stopWords.forEach(w -> stopWordTrie.insert(w, null));
        logger.debug("{} stop words loaded", stopWords.size());
    }
